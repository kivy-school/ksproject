...
class MsvcProjectBuilder:
    ...
    def __init__(self, pyproject: PyProjectToml, working_dir: Path):
        ...
        self.package_name = self.pyproject.project.name.strip().replace("-", "_").replace(" ", "_")
    ...
    def _resolve_and_convert_icon(self, dest_dir: Path) -> Path | None:
        ...
        if Image is None:
            print("[ksproject] Pillow is required to convert icons to .ico. Continuing without icon.")
            return None
    ...
    def generate(self) -> None:
        ...
        app_src_dir = self.working_dir / "src" / self.package_name
        if not app_src_dir.exists():
            app_src_dir = self.working_dir / self.package_name

        site_packages_dir = self.working_dir / "project_dist" / "windows" / "site_packages" / "windows"

        self._copy_venv_dlls(site_packages_dir)

        payload_path = MsvcBuildFiles.create_payload_zip(dist_dir, app_src_dir, site_packages_dir, env_dir)

        MsvcBuildFiles.write_main_c(dist_dir, self.package_name, py_version)
        MsvcBuildFiles.write_resources_rc(dist_dir, payload_path, icon_path)
