...
For each arch in your `archs` list (e.g., `arm64-v8a`, `x86_64`), packages are installed to:
...
During the Gradle build, custom tasks copy these into the APK's assets directory at `assets/site_packages/<arch>/`.

The `uv pip install` command uses:

- `--python-platform` to target the correct OS/arch
- `--index-strategy unsafe-best-match` to prefer pre-built mobile wheels
- Extra index URLs configured in `[tool.uv]` (PySwift and KivySchool channels) for pre-built mobile wheels

---

## The Dot-Directory Convention

After pip installs all packages, ksproject scans for special **dot-prefixed directories** that packages inject via custom build backends ([ksp-builder](../plugins/overview.md), [pyjnius-builder](../plugins/overview.md)). These directories are **extracted from site-packages** and handled specially during the Gradle build.
...
**How they get there:** The [pyjnius-builder](https://github.com/kivy-school/pyjnius-builder) build backend injects Java source files from `[tool.pyjnius].java-paths` into the wheel under `.java/`.
...
**How they get there:** The [ksp-builder](https://github.com/kivy-school/ksp-builder) build backend reads `[tool.kivy-school.android]` from the package's pyproject.toml and injects a `.gradle/<package_name>.json` file into the wheel.
...
The generated `app/build.gradle.kts` includes custom Gradle tasks that copy site-packages and native assets into the APK at build time:

| Task | Purpose |
|------|---------|
| `copySitePackages_<abi>` | Copy per-arch Python packages to assets |
| `copySitePackagesJava` | Copy `.java/` sources into `src/main/java/` |
| `copySitePackagesKotlin` | Copy `.kotlin/` sources into project |
| `copySitePackagesNativeLibs_<abi>` | Copy `.libs/<abi>/*.so` to `jniLibs/` |

These tasks run before the compile phase, ensuring all injected sources and libraries are available during Gradle compilation.
...
│       ├── libSDL2.so             # SDL2 (built by CMake from source)
│       ├── libmain.so             # Native bootstrap (main.c)
│       ├── libcrypto.so           # OpenSSL (from CPython build)
│       ├── libssl.so              # OpenSSL (from CPython build)
│       └── libcustom.so           # Any .libs/ native libraries
├── assets/
│   ├── python3.13/                # Python standard library
│   ├── lib-dynload/arm64-v8a/     # C extension modules (.so)
│   └── site_packages/arm64-v8a/   # Your app + all Python dependencies
├── res/
...
ksproject also handles **local/workspace dependencies** (packages referenced by path in your pyproject.toml). It recursively follows all path dependencies to collect their dot-directories too.
...
| Site-packages (Python) | `site_packages/<abi>/` → copied to `assets/site_packages/<abi>/` at build time | `assets/` in APK |
