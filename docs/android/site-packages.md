...
- Your project's `[tool.uv.pip]` config — the KivySchool wheel indexes plus your local [wheelhouse](../wheelhouse/overview.md)

At build time, Gradle stages these directories together with the Python stdlib and `lib-dynload`, [optimizes them](building.md#size-optimizations), and packs everything into a single `assets.zip` inside the APK.

---

## The Dot-Directory Convention

After pip installs all packages, ksproject scans for special **dot-prefixed directories** that packages inject via custom build backends ([ksp-builder](../plugins/overview.md), [pyjnius-builder](../plugins/overview.md)). These directories are handled specially during the Gradle build instead of being bundled as Python assets.
...
| `optimizeStaged_<abi>` | Byte-compile, strip junk files/dirs, and `llvm-strip` native libs ([details](building.md#size-optimizations)) |
